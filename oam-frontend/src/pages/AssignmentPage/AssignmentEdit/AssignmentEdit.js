...
const AssignmentEdit = () => {
    ...
    useEffect(() => {
        ...
        axios({
            method: 'GET',
            url: `${API.ASSIGNMENT_DETAIL}/${assignmentId}`,
            headers: {
            ...
            if (!isCancel) {
                ...
                setAssignment({
                    ...assignment,
                    user_fullname: userList.find(user => user.staffcode === respone.data.assignedTo).fullName,
                    user_code: respone.data.assignedTo,
                    assetCode: respone.data.assetCode,
                    note: respone.data.note
                });
                setCurrentAsset(respone.data.assetCode)
                setIsLoading(false);
            }
        }).catch((error) => {
            if (!isCancel) {
    ...
    const handleOnSubmit = (evt) => {
        ...
        let isCancel = false;
        if (user) {
            if (asset || assignment.assetCode === currentAsset) {
                axios({
                    method: 'PUT',
                    url: `${API.ASSIGNMENT_EDIT}/${assignmentId}`,
                    data: {
        ...
        if (assignment.assetCode !== currentAsset) {
            if (!asset) {
            ...
            <Form onSubmit={handleOnSubmit}>
                ...
                <Form.Group as={Row} className="mb-3" >
                    ...
                    <Col sm={9}>
                        <InputGroup hasValidation>
                            <Form.Control
                                id="assignment-edit-input-user"
                                value={assignment.user_fullname} onChange={handleInputUserChange}
                                name="user" type="text"
                                isInvalid={!isValidUser}
                            />
                            <Button variant="outline-secondary" id="assignment-edit-find-user-button" onClick={() => setOpenSearchUser(!openSearchUser)}>
                ...
                <Form.Group as={Row} className="mb-3" >
                    ...
                    <Col sm={9}>
                        <InputGroup hasValidation>
                            <Form.Control
                                id="assignment-edit-input-asset"
                                value={assignment.assetCode} onChange={handleInputAssetChange}
                                name="asset" type="text"
                                isInvalid={!isValidAsset}
                            />
                            <Button variant="outline-secondary" id="assignment-edit-find-asset-button" onClick={() => setOpenSearchAsset(!openSearchAsset)}>
                                <i className="fa-solid fa-magnifying-glass"></i>
                            </Button>
                            <Form.Control.Feedback type="invalid">
                ...
                <Form.Group as={Row} className="mb-3" >
                    ...
                    <Col sm={9}>
                        <Form.Control
                            id="assignment-edit-input-assigned-date"
                            name="date" type="date"
                ...
                <Form.Group as={Row} className="mb-3" >
                    ...
                    <Col sm={9}>
                        <Form.Control id="assignment-edit-input-note"
